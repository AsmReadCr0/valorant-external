...
namespace uefi {
	...
	public:
		...
		// km 
		[[nodiscard]] bool read_physical_km(std::uintptr_t address, void* buffer, std::uintptr_t length, std::size_t* bytes);
		...
		auto validate_pointer(auto address)
		{
			if (this->is_guarded(address))
				return this->m_guarded + (address & 0xFFFFFF);
			else
				return address;
		}

		template <typename t>
		[[nodiscard]] auto read_guarded(std::uintptr_t address) -> t {

			t response{ };

			auto result = read_virtua_km(
				address,
				&response,
				sizeof(t)
			);
			if (is_guarded(response))
			{
				response = validate_pointer((uintptr_t)response);
			}
		...
		template<typename T>
		[[nodiscard]] auto read_guarded_2(std::uintptr_t address) -> T {
			T response{};
			auto result = read_physical(address, &response, sizeof(T));
			if constexpr (std::is_same_v<T, sdk::tarray<uintptr_t>>) {
				for (auto& item : response.get_itter()) {
					if (is_guarded(item)) {
						item = validate_pointer((uintptr_t)item);
					}
				}
			}
			else {
